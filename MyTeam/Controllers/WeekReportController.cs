...
namespace MyTeam.Controllers
{
    ...
    public class WeekReportController : BaseController
    {
        ...
        public ActionResult AddDetail()
        {
            ...
            // 工作任务下拉列表：其中还包括所有进度未达到100%、与本人有关的重点工作
            var mains = dbContext.WeekReportMains.Where(a => a.Progress < 100);
            if (!user.IsAdmin)
            {
                mains = mains.Where(a => a.Person.Contains(user.Realname));
            }
            SelectList sl1 = new SelectList(mains, "WorkName", "WorkName");
            SelectList sl2 = MyTools.GetSelectList(Constants.WorkMissionList);
            
            ViewBag.WorkMissionList = sl2;
            ...
            for (int i = 0; i < 5; i++)
            {
                ls.Add(new WeekReportDetail() { Person = user.Realname, RptPersonID = user.UID });
            }
